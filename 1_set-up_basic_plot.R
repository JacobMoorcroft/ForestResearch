...
rm(list=setdiff(ls(), c("raw_extracted_data", "country_names"))) # removes unnecessary variables to clean environment
...
woodland_area<-woodland_area%>% rename(Northern_Ireland=`Northern.Ireland`) # *corrects for interaction issues caused by spacing in `Northern Ireland`
country_names<-c("England", "Wales", "Scotland", "Northern_Ireland") # *

woodland_by_year<-cbind(year_ending_March_31st,woodland_area) # Amount of Woodland Area By Year
...
rm(list=setdiff(ls(), c("processed_extracted_data", "woodland_by_year", "country_names", "percentage_results"))) # removes unnecessary variables to clean environment
